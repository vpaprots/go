...
func cgen_wb(n, res *Node, wb bool) {
	...
	if Ctxt.Arch.Thechar == '7' || Ctxt.Arch.Thechar == '9' || Ctxt.Arch.Thechar == 'z' {
		// if both are addressable, move
		if n.Addable {
...
abop: // asymmetric binary
	...
	if nl.Ullman >= nr.Ullman {
		...
		if Smallintconst(nr) && Ctxt.Arch.Thechar != '5' && Ctxt.Arch.Thechar != '7' && Ctxt.Arch.Thechar != '9' && Ctxt.Arch.Thechar != 'z' { // TODO(rsc): Check opcode for arm
			n2 = *nr
		} else {
...
func bgenx(n, res *Node, wantTrue bool, likely int, to *obj.Prog) {
	...
	case ONAME:
		if genval {
			...
			switch Ctxt.Arch.Thechar {
			case '5', '7', '9', 'z':
				Fatal("genval 5g, 7g, 9g, z ONAMES not fully implemented")
			}
			Cgen(n, res)
			if !wantTrue {
				Thearch.Gins(Thearch.Optoas(OXOR, Types[TUINT8]), Nodintconst(1), res)
		...
		if n.Addable && Ctxt.Arch.Thechar != '5' && Ctxt.Arch.Thechar != '7' && Ctxt.Arch.Thechar != '9' && Ctxt.Arch.Thechar != 'z' {
	...
	if nr.Ullman >= UINF {
		...
		if Smallintconst(nr) && Ctxt.Arch.Thechar != '9' && Ctxt.Arch.Thechar != 'z' {
...
func cgen_div(op int, nl *Node, nr *Node, res *Node) {
	...
	// in peep and optoas in order to enable this.
	if nr.Op != OLITERAL || Ctxt.Arch.Thechar == '7' || Ctxt.Arch.Thechar == '9' || Ctxt.Arch.Thechar == 'z' {
		goto longdiv
	}
